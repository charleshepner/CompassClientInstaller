...
!define PRODUCT_VERSION "2012.05.08"
...
Section "-Pre"
  ...
  ${If} "$AnswerFilePath" == ""
	DetailPrint "No answer file found or path provided does not exist."
  ${Else}
...
Function .onInit
  ...
  ${If} ${FileExists} "$AnswerFilePath"
    ...
    ${EndIf}
  ${Else}
  ; If we get here, no answer file was provided (or path provided did not exist) so read the compiled in constants into the variables for configuration
	StrCpy "$AnswerFilePath" ""
    ...
    StrCpy "$InstallTrustedSites" "${INSTALL_TRUSTED_SITES}"
